 ...
 */
function handleRetry(author: User, retryThreshold: number = config('bot.msgRetryThreshold')) {
 ...
 */
function throttleUser(author: User, callback: () => void, throttleTime: number = config('bot.msgThrottleTime')): void {
...
async function messageEvent(message: Message): Promise<any> {
  ...
  if (content.startsWith(botUsername)) {
    const allowedTags: string[] = [...getTags().keys()].filter((key: string) => key !== '__reserved');
    ...
    try {
      ...
      throttleUser(author, async (): Promise<void> => {
